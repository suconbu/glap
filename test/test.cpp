...
{
    glapp::window window(640, 480, "Hello glapp");
    EXPECT_TRUE(window);
    EXPECT_EQ(window.size().width(), 640);
    EXPECT_EQ(window.size().height(), 480);
    EXPECT_STREQ(window.title().c_str(), "Hello glapp");
    window.set_title(u8"こんにちはglap");
    EXPECT_STREQ(window.title().c_str(), u8"こんにちはglap");
    EXPECT_NE(window.glfw_handle(), nullptr);
    EXPECT_EQ(window.tag().length(), 0);
    window.set_tag("tag");
    EXPECT_STREQ(window.tag().c_str(), "tag");
    int var = 0;
    ...
    EXPECT_EQ(window.size().width(), 0);
}

TEST_F(GlapTest, Get)
{
    glapp::window window(640, 480, nullptr);
    window.on_frame([](glapp::window& w) {
        ...
        w.set_clipboard_string("glapp");
        EXPECT_STREQ(w.clipboard_string().c_str(), "glapp");
        w.set_clipboard_string("");
        EXPECT_STREQ(w.clipboard_string().c_str(), "");
...
{
    glapp::window w(640, 480, nullptr);
...
TEST_F(GlapTest, Error)
{
    ...
    (void)glapp::window(100, 100, nullptr, glapp::window_options().set_opengl_version(0, 0));
...
TEST_F(GlapTest, WindowCallback)
{
    ...
    int draw_count = 0;
    window.on_frame([&](glapp::window& window) {
        ++draw_count;
        window.minimize();
        window.minimize();
        window.restore();
    ...
    window.on_window_state([&](glapp::window&, glapp::window_state state) {
    ...
    window.on_window_focus([&](glapp::window&, bool focused) {
        if (focused) {
    ...
    window.on_window_close([&](glapp::window&) {
        ++window_close_count;
    });
...
TEST_F(GlapTest, WindowStateFromNormal)
{
    ...
    glapp::window w(normal_size.width(), normal_size.height(), nullptr);
    const auto monitor_rect = w.placed_monitor().rect();
    w.maximize();
...
{
    glapp::window w(600, 600, nullptr);
